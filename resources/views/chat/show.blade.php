...
<div class="flex items-center gap-3 mb-6">
    ...
    <div class="w-8 h-8 rounded-full bg-[#f0c36d]/10 border border-[#f0c36d]/20 flex items-center justify-center
                text-[#f0c36d] text-xs font-bold uppercase">
    ...
    <div>
        ...
        @if($conversacion->producto)
            <p class="text-[10px] text-white/30 mt-0.5 uppercase tracking-wider">
                Sobre: {{ $conversacion->producto->titulo }}
            </p>
        @endif
    </div>
</div>
...
{{-- Burbuja de mensajes --}}
<div id="chat-box"
     class="flex flex-col gap-2 overflow-y-auto mb-4 pr-1"
     style="height: 420px;">
    ...
    @forelse($mensajes as $msg)
        ...
        <div class="flex {{ $esMio ? 'justify-end' : 'justify-start' }}">
            <div class="max-w-[70%] px-3 py-2 rounded-[10px] text-xs leading-relaxed
                        {{ $esMio
...
{{-- Formulario de envío --}}
<form action="{{ route('chat.enviar', $conversacion->id) }}" method="POST"
      class="flex gap-2">
    @csrf
    <input type="text"
           name="contenido"
           ...
           required
           class="flex-1 bg-white/5 border {{ $errors->has('contenido') ? 'border-red-500/50' : 'border-white/10' }}
                  rounded-[8px] px-3 py-2 text-xs text-gray-200 placeholder-white/25
    ...
    <button type="submit"
            class="px-4 py-2 rounded-[8px] bg-[#f0c36d]/15 border border-[#f0c36d]/30 text-[#f0c36d]
                   text-xs uppercase tracking-widest font-bold
                   hover:bg-[#f0c36d]/25 hover:border-[#f0c36d]/50 transition-all duration-150 shrink-0">
        Enviar
    </button>
</form>
