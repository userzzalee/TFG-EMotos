...
<main class="relative z-10 text-white pt-[75px] pb-20">
    <div class="max-w-2xl mx-auto px-8">
        ...
        <form action="{{ route('segundamano.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            {{-- Título --}}
            <div>
                <label class="block text-xs tracking-widest text-gray-400 mb-2 uppercase">Título</label>
                <input type="text" name="titulo" value="{{ old('titulo') }}" required class="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-white text-sm focus:border-yellow-500 focus:outline-none transition-all" placeholder="Ej: Casco Shoei GT-Air talla M">
            </div>
            {{-- Descripción --}}
            <div>
                <label class="block text-xs tracking-widest text-gray-400 mb-2 uppercase">Descripción</label>
                <textarea name="descripcion" rows="3" class="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-white text-sm focus:border-yellow-500 focus:outline-none transition-all resize-none" placeholder="Describe el artículo, su uso, posibles defectos…">{{ old('descripcion') }}</textarea>
            </div>
            {{-- Precio --}}
            <div>
                <label class="block text-xs tracking-widest text-gray-400 mb-2 uppercase">Precio (€)</label>
                <input type="number" name="precio" value="{{ old('precio') }}" step="0.01" min="0" required class="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-white text-sm focus:border-yellow-500 focus:outline-none transition-all" placeholder="0.00">
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="estado" required class="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-white text-sm focus:border-yellow-500 focus:outline-none transition-all">
            ...
            <div class="flex gap-3 pt-2">
                <a href="{{ route('segundamano.index') }}" class="flex-1 bg-gray-800 text-white py-2 rounded-lg text-center text-xs tracking-widest uppercase hover:bg-gray-700 transition-all">Cancelar</a>
                <button type="submit" class="flex-1 bg-yellow-500 text-black py-2 rounded-lg text-xs tracking-widest uppercase font-medium hover:bg-yellow-400 transition-all">Publicar</button>
            </div>
        </form>
    </div>
</main>
